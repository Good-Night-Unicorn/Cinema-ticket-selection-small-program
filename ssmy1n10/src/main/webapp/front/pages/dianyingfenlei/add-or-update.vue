	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`dianyingfenlei`, this.ruleForm.id);
...
//跨表计算判断
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`dianyingfenlei`, this.ruleForm);
						}else{
							await this.$api.add(`dianyingfenlei`, this.ruleForm);
						}
