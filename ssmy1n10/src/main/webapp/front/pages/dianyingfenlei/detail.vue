...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','dianyingfenlei');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','dianyingfenlei');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('dianyingfenlei', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdianyingfenlei/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('dianyingfenlei', this.detail);
				this.$utils.msg('审核成功');
